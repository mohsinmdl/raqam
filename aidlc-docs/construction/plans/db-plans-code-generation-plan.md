...
- [x] **Step 2 — Verification script**: Create `scripts/plans-migration-verify.sql` — post-apply checks from business-logic-model.md (default-plan count, zero unstamped rows per table, ownership-integrity join, per-table row counts to compare against a pre-apply snapshot; plus a pre-apply snapshot query block).
