...
**Migration 0010 must be run in the SQL editor before this code is deployed** — it widens the audit_log CHECKs for the undo/redo rows. Without it, an undo/redo row's CHECK violation throws mid-push (audit is pushed last, and inserts for every collection run before any deletes): the push aborts, the poisoned row stays queued forever, and no deletes sync at all for the rest of the session — not just undo/redo's, everyone's. Recoverable by reload (audit is skipFetch), never corrupts the server, but it is not a narrow undo-only failure.
